...
export interface MobileLayout<T> {
  ...
  /** Container width breakpoint (px) for switching to desktop. Default: "640px". */
  breakpoint?: string;
}
...
export function DataTable<T>({
  ...
  const bp = mobile ? parseInt(mobile.breakpoint ?? "640", 10) : 0;
  const [setContainer, wide] = useContainerWide(bp);
  const [sorting, setSorting] = useState<SortingState>(defaultSort);
  const [expandedId, setExpandedId] = useState<string | null>(null);

  const activeColumns = mobile && !wide ? mobile.columns : columns;
  const activeExpand = mobile && !wide ? mobile.expandedContent : expandedContent;
      ...
      <Table className={tableClassName}>
        ...
        <TableHeader>
          {table.getHeaderGroups().map((hg) => (
            <TableRow key={hg.id}>
              {hasExpand && <TableHead className="w-12" />}
        ...
        <TableBody>
          {table.getRowModel().rows.map((row) => {
            ...
            const detail = activeExpand?.(row.original);
