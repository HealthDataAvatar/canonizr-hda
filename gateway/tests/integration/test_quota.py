...
class TestQuotaEnforcement:
    ...
    def test_under_quota_allows_request(self, test_sub, r, table):
        ...
        _set_quota_in_table(table, test_sub, 100_000)
        resp = _convert(b"hello", test_sub)
        assert resp.status_code == 202

    def test_over_quota_rejects(self, test_sub, r, table):
        """Usage already at limit → 429."""
        _set_quota_in_table(table, test_sub, 1)
    ...
    def test_natural_quota_exhaustion(self, test_sub, r, table):
        ...
        _set_quota_in_table(table, test_sub, 100)
        # First: 5 bytes
        resp1 = _convert(b"hello", test_sub)
        assert resp1.status_code == 202
        usage = int(r.get(f"sub:{test_sub}:bytes") or 0)
        assert usage == 5
        # Fill up the rest
        resp2 = _convert(b"x" * 95, test_sub)
        assert resp2.status_code == 202
        # Now at 100/100 — next request should be rejected
        resp3 = _convert(b"y", test_sub)
    ...
    def test_file_larger_than_total_quota(self, test_sub, r, table):
        ...
        _set_quota_in_table(table, test_sub, 10)
        resp = _convert(b"x" * 100, test_sub)
        assert resp.status_code == 429
    ...
    def test_quota_loaded_from_table_on_cache_miss(self, test_sub, r, table):
        ...
        r.delete(f"sub:{test_sub}:quota:bytes")
        ...
        cached = r.get(f"sub:{test_sub}:quota:bytes")
