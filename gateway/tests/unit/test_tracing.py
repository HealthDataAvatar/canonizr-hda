...
def test_to_steps_single_service():
    trace = Trace("worker")
    with trace.span("liteparse", input_bytes=100, pages=3):
        pass
    trace.finish()
...
def test_to_steps_skips_internal_spans():
    ...
    assert len(steps) == 1
    assert steps[0].service == "liteparse"
