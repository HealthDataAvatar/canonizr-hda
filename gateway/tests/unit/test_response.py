...
def test_metadata_includes_consumption_fields():
    result = ConvertResult(markdown="hello", input_kb=12.3, images_captioned=2, images_skipped=1, images_errored=0, images_failed=1)
    meta = result._metadata()
    assert meta["input_kb"] == 12.3
...
def test_metadata_defaults_zero():
    ...
    meta = result._metadata()
    assert meta["input_kb"] == 0.0
    assert meta["images_errored"] == 0
    assert meta["images_failed"] == 0


def test_metadata_json_includes_input_kb():
    result = ConvertResult(markdown="x", input_kb=5.5)
    parsed = json.loads(result.metadata_json())
    assert parsed["input_kb"] == 5.5


def test_to_dict_includes_metadata():
    result = ConvertResult(markdown="x", input_kb=1.0, images_failed=3)
    d = result.to_dict()
    assert d["metadata"]["input_kb"] == 1.0
